...
MR1.1.loc<-lm(rich~std.conservation+std.vegetation+std.PCA.wclim.1+std.PCA.wclim.2,data=decostand(environment,"standardize"))

MC1.1.loc<-lm(pcoa.jac.1~std.conservation+std.vegetation+std.PCA.wclim.1+std.PCA.wclim.2,data=decostand(environment,"standardize"))

MC2.1.loc<-lm(pcoa.jac.2~std.conservation+std.vegetation+std.PCA.wclim.1+std.PCA.wclim.2,data=decostand(environment,"standardize"))
#summary(MR1.1.loc)
#summary(MC1.1.loc)
#summary(MC2.1.loc)
...
MR1.1<-lm(rich~std.conservation+std.vegetation+std.PCA.wclim.1+std.PCA.wclim.2,data=decostand(environment.AF.2d,"standardize"))
...
MR1.6<-lm(MR1.4$residuals~std.conservation+std.vegetation+std.PCA.wclim.1+std.PCA.wclim.2,data=decostand(environment.2d,"standardize"))

MR1.7<-lm(MR1.5$residuals~std.conservation+std.vegetation+std.PCA.wclim.1+std.PCA.wclim.2,data=decostand(environment.2d,"standardize"))
...
MC1.1<-lm(pcoa.jac.1~std.conservation+std.vegetation+std.PCA.wclim.1+std.PCA.wclim.2,data=decostand(environment.AF.2d,"standardize"))
...
MC1.6<-lm(MC1.4$residuals~std.conservation+std.vegetation+std.PCA.wclim.1+std.PCA.wclim.2,data=decostand(environment.2d,"standardize"))

MC1.7<-lm(MC1.5$residuals~std.conservation+std.vegetation+std.PCA.wclim.1+std.PCA.wclim.2,data=decostand(environment.2d,"standardize"))
...
MC2.1<-lm(pcoa.jac.2~std.conservation+std.vegetation+std.PCA.wclim.1+std.PCA.wclim.2,data=decostand(environment.AF.2d,"standardize"))
...
MC2.6<-lm(MC2.4$residuals~std.conservation+std.vegetation+std.PCA.wclim.1+std.PCA.wclim.2,data=decostand(environment.2d,"standardize"))

MC2.7<-lm(MC2.5$residuals~std.conservation+std.vegetation+std.PCA.wclim.1+std.PCA.wclim.2,data=decostand(environment.2d,"standardize"))
...
sMSE=c(MidD=s.mse.MidD,Neutral=s.mse.neutral,s.mse.logis))[c(1:2,5:6,26:27),]
...
```
  ...
  In the small grain (original sampling units), the bioclim variables were more important predictors than conservation or vegetation. However, in this scale, the environmental variables were able to explain just 17% of the variation in species richness.
...
print(xtable(AIC(MR1.1,MR1.2,MR1.3,MR1.4,MR1.5,MR1.6,MR1.7,MR1.8,MR1.9,MR1.10)),type="html",html.table.attributes="border=0, bgcolor=#989898")
...
sMSE=c(MidD=s.mse.MidD.jac,Neutral=s.mse.neu.jac,s.mse.logis.jac))[c(1:2,5:6,26:27),]
),type="html",html.table.attributes="border=0, bgcolor=#989898")
...
```
 ...
 Converselly, three of the four the best regression models for the first ordination axis were obtained including the neutral results as a predictor variable (Table 4). The fourth best model was obtained using just the logistic model as a predictor. For the second axis, the best models included the environmental predictors alone, or in combination with the Neutral or Mid Domain results. The neutral and Mid Domain models alone were able to explain 78% and 77% of the variation in the first axis of species composition. The environmental variables combined explained 77% of the variation is the first axis. The combination of environmental and neutral predictors gave an explanation of 81% for the first ordination axis. The environmental variables combined with the neutral model explained 49% of the variation in the second ordination axis. The use of the Mid Domain instead of the neutral predictor gave similar results (R2 = 0.48; Table 4).
  ...
  Similarly to species richness, the bioclim variables were the most important environmental predictors of species composition for both ordination axes. The environment alone was able to explain 61% and 32% of the variation in the first two ordination axes of species composition, respectivelly.
...
```{r Fig 3. distance decay real.midDomain and neutral mor, echo=FALSE,results='hide',warning=FALSE}
...
```{r Fig S1. Map Richness Mid Domain vs Neutral,fig.width=18,fig.height=6,echo=FALSE,message=FALSE,results='hide',warning=FALSE}
...
```{r Fig S2. rich.logis in the map, echo=FALSE, message=FALSE,warning=FALSE}
...
```{r Fig S3. Map Composition Mid Domain vs Neutral,fig.width=18,fig.height=6,echo=FALSE,message=FALSE,results='hide',warning=FALSE}
...
```{r Fig S4. jac.logis in the map, echo=FALSE, message=FALSE,warning=FALSE}
...
```{r Fig S5. plot all predictions against lat, fig.width=16,fig.height=8,message=FALSE,results='hide',echo=FALSE,warning=FALSE}

par(mfrow=c(1,2))

plot(environment.2d$Lat2,decostand(environment.2d$pcoa.jac.1,'range'),bg="gold",pch=22,xlab="Latitude",ylab="Axis 1")
...
``` {r Table S2. MSE - rich Results,echo=FALSE,results='asis'}
...
```{r Table S3. MSE - comp Results,echo=FALSE,results='asis'}
...
```{r Hill.numbers on the map - logistic, echo=FALSE,warning=FALSE,message=FALSE}
################################
...
#   title(main=i,line=-3)
#   points(Long2,Lat2,pch=22,bg='darkgrey',col=0,cex=.5)
#   rect(Long2-1,Lat2-1,Long2+1,Lat2+1,col=color.select(Hill.logis[[paste("Hill.logis.",i,sep="")]],palette=Custom.Palette))
#   plot(brazil,add=T)
#   }
...
```{r plot logis hill,fig.align='center',fig.width=8,fig.height=6, echo=FALSE,warning=FALSE,message=FALSE}
...
for (i in glm.variables.2d){plot(Hill.logis[[paste("Hill.logis.",i,sep="")]]~environment.2d[[i]],pch=21,cex=1,bg=color.select(Hill.logis[[paste("Hill.logis.",i,sep="")]]),xlab=i,ylab="Diversity",axes=FALSE);box()}

```
#### Fig S6.

```{r plot logis rich and srich,echo=FALSE,results='hide',fig.height=6,fig.width=8,warning=FALSE}
...
```{r comparisons of the models - richness, echo=FALSE,eval=FALSE}
...
```{r species composition plot, echo=FALSE,eval=FALSE}
...
```{r observed vs predicted, echo=FALSE,eval=FALSE}
