...
envdist.loc<-dist(environment[grep("std",colnames(environment))][c(25,26)])
...
envdist.2d<-dist(environment.2d[grep("std",colnames(environment.2d))][c(25,26)]) #Using just PCA1 and 2 of wclim var
...
MR1.1.loc<-glm(rich~PCA.wclim.1+PCA.wclim.2,data=decostand(environment,"standardize"))

MC1.1.loc<-glm(pcoa.jac.1~PCA.wclim.1+PCA.wclim.2,data=decostand(environment,"standardize"))

MC2.1.loc<-glm(pcoa.jac.2~PCA.wclim.1+PCA.wclim.2,data=decostand(environment,"standardize"))
...
MR1.1<-glm(rich~PCA.wclim.1+PCA.wclim.2,data=decostand(environment.AF.2d,"standardize"))
...
MR1.6<-glm(MR1.4$residuals[environment.AF.2d$Lat2!=100]~PCA.wclim.1+PCA.wclim.2,data=decostand(environment.AF.2d,"standardize"))
MR1.7<-glm(MR1.5$residuals[environment.AF.2d$Lat2!=100]~PCA.wclim.1+PCA.wclim.2,data=decostand(environment.AF.2d,"standardize"))

MR1.8<-glm(rich~0+rich.logis.PCA.wclim.1+rich.logis.PCA.wclim.2,data=decostand(environment.AF.2d,"standardize"))
...
MC1.1<-glm(pcoa.jac.1~std.PCA.wclim.1+std.PCA.wclim.2,data=decostand(environment.AF.2d,"standardize"))
...
MC1.6<-glm(MC1.4$residuals[environment.AF.2d$Lat2!=100]~PCA.wclim.1+PCA.wclim.2,data=decostand(environment.AF.2d,"standardize"))
MC1.7<-glm(MC1.5$residuals[environment.AF.2d$Lat2!=100]~PCA.wclim.1+PCA.wclim.2,data=decostand(environment.AF.2d,"standardize"))

MC1.8<-glm(pcoa.jac.1~0+pcoa.logis.jac.PCA.wclim.1.1+pcoa.logis.jac.PCA.wclim.2.1,data=decostand(environment.AF.2d,"standardize"))
...
MC2.1<-glm(pcoa.jac.2~PCA.wclim.1+PCA.wclim.2,data=decostand(environment.AF.2d,"standardize"))
...
MC2.6<-glm(MC2.4$residuals[environment.AF.2d$Lat2!=0]~PCA.wclim.1+PCA.wclim.2,data=decostand(environment.AF.2d,"standardize"))

MC2.7<-glm(MC2.5$residuals[environment.AF.2d$Lat2!=0]~PCA.wclim.1+PCA.wclim.2,data=decostand(environment.AF.2d,"standardize"))

MC2.8<-glm(pcoa.jac.2~0+pcoa.logis.jac.PCA.wclim.1.2+pcoa.logis.jac.PCA.wclim.2.2,data=decostand(environment.AF.2d,"standardize"))
...
``` {r Tab1_MSE - rich Results,echo=FALSE,results='asis'}
...
sMSE=c(MidD=s.mse.MidD,Neutral=s.mse.neutral,s.mse.logis))[c(1:2,5,26,27),]
...
Similarly, the regression model with the lowest AIC was the linear regression of the species richness against habitat quality (Table 2). However, the inclusion of the neutral model prediction in the same model produced a similar AIC value and increased the explanatory power in about 7% (Table 2). Habitat quality alone was able to explain `r round(cor(MR1.11$fitted.values+MR1.11$residuals,MR1.11$fitted.values)^2*100,0)`% of the variation in species richness. The climatic variables were able to explain `r round(cor(MR1.1$fitted.values+MR1.1$residuals,MR1.1$fitted.values)^2*100,0)`% of the variation on species richness. The logistic models had similar explanatory power to the models using the raw environmental predictors.
...
```{r Tab2_AICs richness, echo=FALSE, results='asis'}

npar=c(4,6,4,3,1,6,4,129,131,129,3,5,3,5,3,65,67,65)
...
print(xtable(data.frame(model=strsplit("MR1.1,MR1.2,MR1.3,MR1.4,MR1.5,MR1.6,MR1.7,MR1.8,MR1.9,MR1.10,MR1.11,MR1.12,MR1.13,MR1.16,MR1.17,MR1.18,MR1.19,MR1.20",",")[[1]],Var=c("Clim","Clim+Neu","Clim+Mid","Neu","Mid","ResN~Clim","ResN~Clim","LogisC","LogisC+Neu","LogisC+Mid","Hab","Hab+Neu","Hab+Mid","ResN~Hab","ResN~Hab","LogisH","LogisH+Neu","LogisH+Mid"),logLik=logLik,npar=npar,AIC=-2*logLik+2*npar,rsquares=rsquares)),type=table.type,html.table.attributes="border=1, bgcolor=#989898",comment=FALSE)
...
```{r Tab3_MSE - comp Results,echo=FALSE,results='asis'}
...
sMSE=c(MidD=s.mse.MidD.jac,Neutral=s.mse.neu.jac,s.mse.logis.jac))[c(1:2,5,26,27),]
),type=table.type,html.table.attributes="border=1, bgcolor=#989898",comment=FALSE)
...
Conversely, the best regression models for the first ordination axis were obtained including the mid domain or neutral model results as predictor variables (Table 4). For the second ordination axis, the best model included habitat quality, and habitat quality along the mid domain model. The neutral and mid domain models alone were able to explain `r round(cor(MC1.4$fitted.values+MC1.4$residuals,MC1.4$fitted.values)^2*100,0)`% and `r round(cor(MC1.5$fitted.values+MC1.5$residuals,MC1.5$fitted.values)^2*100,0)`% of the variation in the first axis of species composition. The climatic variables combined explained `r round(cor(MC1.1$fitted.values+MC1.1$residuals,MC1.1$fitted.values)^2*100,0)`% of the variation is the first axis. Habitat quality alone was a poor predictor of the species composition in the first ordination axis, but the best explanatory model was obtained by combining habitat quality and one of the dispersal models (R²=`r round(cor(MC1.13$fitted.values+MC1.13$residuals,MC1.13$fitted.values)^2*100,0)`%). 
For the second axis, the mid domain model along habitat quality explained about `r round(cor(MC2.13$fitted.values+MC2.13$residuals,MC2.13$fitted.values)^2*100,0)`% of the variation in species composition (Table 4). Besides the largest AICs due to the high number of parameters, the combination of the dispersal models with the climatic variables in the logistic regressions gave the best explanatory power for the second ordination axis (R²=`r round(cor(MC1.8$fitted.values+MC1.8$residuals,MC1.8$fitted.values)^2*100,0)`%; Table 4). 
...
```{r Tab4_AICs composition, echo=FALSE, results='asis'}
...
print(xtable(data.frame(model=strsplit("MC1.1,MC1.2,MC1.3,MC1.4,MC1.5,MC1.6,MC1.7,MC1.8,MC1.9,MC1.10,MC1.11,MC1.12,MC1.13,MC1.16,MC1.17,MC1.18,MC1.19,MC1.20,MC2.1,MC2.2,MC2.3,MC2.4,MC2.5,MC2.6,MC2.7,MC2.8,MC2.9,MC2.10,MC2.11,MC2.12,MC2.13,MC2.16,MC2.17,MC2.18,MC2.19,MC2.20",",")[[1]],Var=c("Clim","Clim+Neu","Clim+Mid","Neu","Mid","ResN~Clim","ResN~Clim","LogisC","LogisC+Neu","LogisC+Mid","Hab","Hab+Neu","Hab+Mid","ResN~Hab","ResN~Hab","LogisH","LogisH+Neu","LogisH+Mid"),logLik=logLik,npar=npar,AIC=-2*logLik+2*npar,rsquares=rsquares)),type=table.type,html.table.attributes="border=1, bgcolor=#989898",comment=FALSE)
