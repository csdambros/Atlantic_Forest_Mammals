...
```
  ...
  We analyzed the data using the original local sampling units and grouping the original data points in 26 2x2 degrees grid cells (Fig. 1). Most localities were surveyed by a single study, then most of the sampling points represent a single study (see data collection). We used the number of species encountered in each locality (S), and the similarity in species composition as response variables.
...
envdist.loc<-dist(environment[grep("std",colnames(environment))][c(30,31)])
...
envdist.2d<-dist(environment.2d[grep("std",colnames(environment.2d))][c(30,31)]) #Using just PCA1 and 2 of wclim var
...
####
  ...
  For estimating the influence of dispersal limitation on species local diversity (S) and turnover (Jaccard index; PCoA1 and PCoA2), we simulated the AF as a network of interconnected grid cells where the species or individuals could move between adjacent grid cells (Fig. S2). We used two models to recreate the species distribution under dispersal alone: the Mid-Domain spreading-dye model (Colwell and Hurtt 1994; Colwell and Lees 2000), and the analytical neutral approach borrowed from the population genetics (Nagylaki, 1980) and presented by Economo & Keitt (2008) for community ecology. These models were created just using the grouped data in grid cells because of the completeness of data, and due to computational limitation.
  In the Mid-Domain spreading dye model, we recorded the number of grid cells occupied by each small mammal species. For each species, one of the 26 grid cells was randomly selected and the species occurrence was spread from the selected quadrant to neighboring grid cells until the original number of grid cells was occupied. This procedure was repeated 10,000 times for the `r ncol(species.2d)` species. Each quadrant had up to eight neighbors (Moore neighborhood; Fig. S2), and the model was bounded by the domain where actual small mammal data was recorded (26 grid cells).
...
```
  ...
  In the neutral model, the whole area comprising the AF was divided in 56 2x2 degrees grid cells from which 26 had small mammal data available (Fig. 1). The model started with a single species occupying all the 56 grid cells. In each generation, new species were added in each quadrant by point speciation with rate v, set the same for all grid cells (see Economo & Keitt (2010) and Muneepeerakul et al. (2008) for more details and other uses). v represents the probability of an individual to become a new species but could also represent the addition of new species by immigration from a larger species pool outside the AF (eg. the Cerrado or Amazonian forests). To recreate the dispersal of individuals, we determined that a quadrant could just be colonized by a neighbor (Moore neighborhood; Fig S2), and that all grid cells had the same migration rate (parameter m). The local community size (number of individuals) was set the same for all grid cells (N = 100). The model was run for multiple generations, until the diversity within (PIE and HillPIE) and among grid cells (Morisita-Horn similarity) reached a steady-state (usually more than 30,000 generations).


  Differently from the Mid-domain model, the values of m, v, and N were initially set independently of the observed data. Because different combinations of these parameters can create the same patterns of species distribution (Etienne 2006?; Economo and Keitt 2010), and because estimates for these parameters based on empirical data are not available, we did not attempt to estimate realistic parameters, but focused on the general patterns that the model could produce (see discussion for implications of this procedure). N was arbitrarily defined as 100 individuals for all grid cells, and m and v were used as knobs to best fit the model to the observed similarity in species composition. These parameters were simultaneously tunned using the L-BFSG-S optimization algorithm. In summary, m and v were adjusted to minimize the differences between the predictions of the neutral model to the observed data, giving the best possible explanation of dispersal to the observed data (this procedure is conceptually analog to a regression analysis using maximum likelihood optimization). Because the number of individuals per quadrant was kept constant during the optimization, the differences in diversity were due to the location of a quadrant in the network and the number of neighbor grid cells, but not affected by the differences in abundance.
...
MR1.1.loc<-glm(rich~PCA.wclim.1+PCA.wclim.1,data=decostand(environment,"standardize"))

MC1.1.loc<-glm(pcoa.jac.1~PCA.wclim.1+PCA.wclim.1,data=decostand(environment,"standardize"))

MC2.1.loc<-glm(pcoa.jac.2~PCA.wclim.1+PCA.wclim.1,data=decostand(environment,"standardize"))
...
# 
# MR1.1<-glm(rich~std.conservation+std.vegetation+std.PCA.wclim.1+std.PCA.wclim.2,data=decostand(environment.AF.2d,"standardize"))
# 
# MR1.1<-glm(rich~std.conservation,data=decostand(environment.AF.2d,"standardize"))
# 
# summary(MR1.1)
# 
...
# 
# MR1.6<-glm(MR1.4$residuals~std.conservation+std.vegetation+std.PCA.wclim.1+std.PCA.wclim.2,data=decostand(environment.2d,"standardize"))
# 
# MR1.7<-glm(MR1.5$residuals~std.conservation+std.vegetation+std.PCA.wclim.1+std.PCA.wclim.2,data=decostand(environment.2d,"standardize"))
# 
# MR1.8<-glm(rich~rich.logis.conservation+rich.logis.vegetation+rich.logis.PCA.wclim.1+rich.logis.PCA.wclim.2,data=decostand(environment.AF.2d,"standardize"))
# 
# MR1.9<-update(MR1.8,~.+rich.neutral.mean)
# MR1.10<-update(MR1.8,~.+rich.MidD.mean)
# 
# ##### Composition
# 
# MC1.1<-glm(pcoa.jac.1~std.conservation+std.vegetation+std.PCA.wclim.1+std.PCA.wclim.2,data=decostand(environment.AF.2d,"standardize"))
# 
# MR1.1<-glm(rich~std.bio1,data=decostand(environment.AF.2d,"standardize"))
# 
# MC1.1<-glm(pcoa.jac.1~std.bio1+std.bio12,data=decostand(environment.AF.2d,"standardize"))
# 
# 
...
# 
# MC1.6<-glm(MC1.4$residuals~std.conservation+std.vegetation+std.PCA.wclim.1+std.PCA.wclim.2,data=decostand(environment.2d,"standardize"))
# 
# MC1.7<-glm(MC1.5$residuals~std.conservation+std.vegetation+std.PCA.wclim.1+std.PCA.wclim.2,data=decostand(environment.2d,"standardize"))
# 
# MC1.8<-glm(pcoa.jac.1~pcoa.logis.jac.conservation.1+pcoa.logis.jac.vegetation.1+pcoa.logis.jac.PCA.wclim.1.1+pcoa.logis.jac.PCA.wclim.2.1,data=decostand(environment.AF.2d,"standardize"))
# 
# MC1.9<-update(MC1.8,~.+pcoa.jac.neutral.1)
# MC1.10<-update(MC1.8,~.+pcoa.MidD.jac.1)
# 
# 
# #####
# 
# MC2.1<-glm(pcoa.jac.2~std.conservation+std.vegetation+std.PCA.wclim.1+std.PCA.wclim.2,data=decostand(environment.AF.2d,"standardize"))
# 
...
# 
# MC2.6<-glm(MC2.4$residuals~std.conservation+std.vegetation+std.PCA.wclim.1+std.PCA.wclim.2,data=decostand(environment.2d,"standardize"))
# 
# MC2.7<-glm(MC2.5$residuals~std.conservation+std.vegetation+std.PCA.wclim.1+std.PCA.wclim.2,data=decostand(environment.2d,"standardize"))
# 
# MC2.8<-glm(pcoa.jac.2~pcoa.logis.jac.conservation.2+pcoa.logis.jac.vegetation.2+pcoa.logis.jac.PCA.wclim.1.2+pcoa.logis.jac.PCA.wclim.2.2,data=decostand(environment.AF.2d,"standardize"))
# 
# MC2.9<-update(MC2.8,~.+pcoa.jac.neutral.2)
# MC2.10<-update(MC2.8,~.+pcoa.MidD.jac.2)


# just using bio1 and bio2
#########################################

MR1.1<-glm(rich~bio1+bio2,data=decostand(environment.AF.2d,"standardize"))
...
MR1.6<-glm(MR1.4$residuals[environment.AF.2d$Lat2!=100]~bio1+bio2,data=decostand(environment.AF.2d,"standardize"))
MR1.7<-glm(MR1.5$residuals[environment.AF.2d$Lat2!=100]~bio1+bio2,data=decostand(environment.AF.2d,"standardize"))

MR1.8<-glm(rich~0+rich.logis.bio1+rich.logis.bio12,data=decostand(environment.AF.2d,"standardize"))

#MR1.8<-glm(rich~0+offset(.5*rich.logis.bio1)+offset(.5*rich.logis.bio12),data=decostand(environment.AF.2d,"standardize"))

#cor(MR1.1$fitted.values+MR1.1$residuals,MR1.1$fitted.values)
...
MC1.6<-glm(MC1.4$residuals[environment.AF.2d$Lat2!=100]~bio1+bio2,data=decostand(environment.AF.2d,"standardize"))
MC1.7<-glm(MC1.5$residuals[environment.AF.2d$Lat2!=100]~bio1+bio2,data=decostand(environment.AF.2d,"standardize"))

MC1.8<-glm(pcoa.jac.1~0+pcoa.logis.jac.bio1.1+pcoa.logis.jac.bio2.1,data=decostand(environment.AF.2d,"standardize"))
#MC1.8<-glm(pcoa.jac.1~0+offset(.5*pcoa.logis.jac.bio1.1)+offset(.5*pcoa.logis.jac.bio2.1),data=decostand(environment.AF.2d,"standardize"))

#cor(MC1.8$fitted.values+MC1.8$residuals,MC1.8$fitted.values)^2
#cor(MC1.1$fitted.values+MC1.1$residuals,MC1.1$fitted.values)^2
...
MC2.1<-glm(pcoa.jac.2~bio1+bio2,data=decostand(environment.AF.2d,"standardize"))
...
MC2.6<-glm(MC2.4$residuals[environment.AF.2d$Lat2!=0]~bio1+bio2,data=decostand(environment.AF.2d,"standardize"))

MC2.7<-glm(MC2.5$residuals[environment.AF.2d$Lat2!=0]~bio1+bio2,data=decostand(environment.AF.2d,"standardize"))

MC2.8<-glm(pcoa.jac.2~0+pcoa.logis.jac.bio1.2+pcoa.logis.jac.bio2.2,data=decostand(environment.AF.2d,"standardize"))

#MC2.8<-glm(pcoa.jac.2~0+offset(.5*pcoa.logis.jac.bio1.2)+offset(.5*pcoa.logis.jac.bio2.2),data=decostand(environment.AF.2d,"standardize"))
...
#MS1.2<-glm((1-similarity.jac)~envdist.2d+neutral.s,family = binomial(link=log))
#MS1.3<-glm((1-similarity.jac)~envdist.2d+as.dist(1-similarity.MidD.mean),family = binomial(link=log))

MS1.4<-glm((1-similarity.jac)~0+offset(1*neutral.s))
MS1.5<-glm((1-similarity.jac)~0+offset(1*as.dist(1-similarity.MidD.mean)))

#MS1.4<-glm((1-similarity.jac)~0+neutral.s,family = binomial(link=logit))
#MS1.5<-glm((1-similarity.jac)~0+as.dist(1-similarity.MidD.mean),family = binomial(link=probit))

#plot((1-similarity.jac)~neutral.s)
#lines(neutral.s,predict(MS1.4,type="response"))

#plot((1-similarity.jac)~geodist.2d)
#lines(geodist.2d,predict(MS1.0,type="response"))

# summary(MS1.0)
#cor((1-similarity.jac),predict(MS1.0))^2
# summary(MS1.1)
#cor((1-similarity.jac),predict(MS1.1))^2
# summary(MS1.2)
# summary(MS1.3)
#summary(MS1.4)
#cor((1-similarity.jac),predict(MS1.4))^2
# summary(MS1.5)
# 
#cor((1-similarity.jac),neutral.s)^2
# cor((1-similarity.jac),geodist.2d)
#cor((1-similarity.jac),as.dist(1-similarity.MidD.mean))^2
# cor((1-similarity.jac),envdist.2d)
...
sMSE=c(MidD=s.mse.MidD,Neutral=s.mse.neutral,s.mse.logis))[c(1:2,7,18),]
...
###
  ...
  The regression model with the lowest AIC was the linear regression of the species richness against the raw environmental predictors (bio1 and bio2) (Table 2). However, the inclusion of the neutral and Mid-Domain predictions along the raw environmental variables gave similar results (Table 2). The environmental variables were able to explain `r round(cor(MR1.1$fitted.values+MR1.1$residuals,MR1.1$fitted.values)^2*100,0)`% of the variation on species richness, and `r names(MR1.1$coef[-1])[order(MR1.1$coef[-1],decreasing=T)][1]` had the strongest effect. The logistic model had a similar explanatory power to the model usng the raw environmental predictors.
  Besides the poor fit for the data, the Mid Domain and Neutral models created the well known richness peak in the central areas of the Atlantic forest (Fig. 2). The neutral and Mid Domain models were able to explain alone `r round(cor(MR1.4$fitted.values+MR1.4$residuals,MR1.4$fitted.values)^2*100,0)`% and `r round(cor(MR1.5$fitted.values+MR1.5$residuals,MR1.5$fitted.values)^2*100,0)`% of the variation on species richness, respectively.
  In the small grain (original sampling units), the bioclim variables were able to explain `r round(cor(MR1.1.loc$fitted.values+MR1.1.loc$residuals,MR1.1.loc$fitted.values)^2*100,0)`% of the variation in species richness.
...
```{r AICs richness, echo=FALSE, results='asis'}

npar=c(4,6,4,3,1,6,4,129,131,129)

logLik<-unlist(lapply(list(MR1.1,MR1.2,MR1.3,MR1.4,MR1.5,MR1.6,MR1.7,MR1.8,MR1.9,MR1.10),logLik))

rsquares<-unlist(lapply(list(MR1.1,MR1.2,MR1.3,MR1.4,MR1.5,MR1.6,MR1.7,MR1.8,MR1.9,MR1.10),function(x)cor(x$fitted.values+x$residuals,x$fitted.values)^2))

#rsquares<-unlist(lapply(list(MR1.1,MR1.2,MR1.3,MR1.4,MR1.5,MR1.6,MR1.7,MR1.8,MR1.9,MR1.10),Nagr2))

print(xtable(data.frame(Model=paste("MR1.",1:10,sep=""),logLik=logLik,npar=npar,AIC=-2*logLik+2*npar,rsquares=rsquares)),type=table.type,html.table.attributes="border=1, bgcolor=#989898",comment=FALSE)

#summary(MR1.1.loc)

#print(xtable(AIC(MR1.1,MR1.2,MR1.3,MR1.4,MR1.5,MR1.6,MR1.7,MR1.8,MR1.9,MR1.10)),type=table.type,html.table.attributes="border=1, bgcolor=#989898",comment=FALSE)
...
title(main="Environment",line=-15,cex.main=3)
...
rect(environment.AF.2d$AF.Long2-1,environment.AF.2d$AF.Lat2-1,environment.AF.2d$AF.Long2+1,environment.AF.2d$AF.Lat2+1,
     col=adjustcolor(color.select(environment.AF.2d$rich.neutral.mean,ref=environment.AF.2d$rich.neutral.mean*environment.AF.2d$Long2/environment.AF.2d$Long2,palette=Custom.Palette),alpha=.8))
...
par(op)
```
#### Fig 2. Projection of the species richness predicted by dispersal models on the map and comparison with the observed species distribution. Warmer colors represent areas with hiher species richness. (A) Observed species richness; (B) Predicted by the environmental variables in the logistic models; (C) Predicted by the Mid Domain model; (D) Predicted by the neutral model. Shadded grid cells represent areas included in the neutral model but where actual data is not available.
...
sMSE=c(MidD=s.mse.MidD.jac,Neutral=s.mse.neu.jac,s.mse.logis.jac))[c(1:2,7,18),]
),type=table.type,html.table.attributes="border=1, bgcolor=#989898",comment=FALSE)
...
Converselly, the best regression models for the first ordination axis were obtained including the Mid Domain or neutral model results as predictor variables (Table 4). For the second ordination axis, the best model included the environmental predictors alone. The neutral and Mid Domain models alone were able to explain `r round(cor(MC1.4$fitted.values+MC1.4$residuals,MC1.4$fitted.values)^2*100,0)`% and `r round(cor(MC1.5$fitted.values+MC1.5$residuals,MC1.5$fitted.values)^2*100,0)`% of the variation in the first axis of species composition. The environmental variables combined explained `r round(cor(MC1.1$fitted.values+MC1.1$residuals,MC1.1$fitted.values)^2*100,0)`% of the variation is the first axis. The combination of environmental and neutral predictors gave an explanation of `r round(cor(MC1.2$fitted.values+MC1.2$residuals,MC1.2$fitted.values)^2*100,0)`% for the first ordination axis. For the second axis, the environmental variables along with the neutral model explained `r round(cor(MC2.2$fitted.values+MC2.2$residuals,MC2.2$fitted.values)^2*100,0)`% of the variation in species composition. The use of the Mid Domain instead of the neutral predictor gave similar results (R2 = `r round(cor(MC2.3$fitted.values+MC2.3$residuals,MC2.3$fitted.values)^2*100,0)`%; Table 4). 
The logistic model explained `r round(cor(MC1.8$fitted.values+MC1.8$residuals,MC1.8$fitted.values)^2*100,0)`% and `r round(cor(MC2.8$fitted.values+MC2.8$residuals,MC2.8$fitted.values)^2*100,0)`% of the variation in the first two ordination axes, respectively.
...
```{r AICs composition, echo=FALSE, results='asis'}

npar=c(4,6,4,3,1,6,4,129,131,129)

logLik<-unlist(lapply(list(MC1.1,MC1.2,MC1.3,MC1.4,MC1.5,MC1.6,MC1.7,MC1.8,MC1.9,MC1.10,MC2.1,MC2.2,MC2.3,MC2.4,MC2.5,MC2.6,MC2.7,MC2.8,MC2.9,MC2.10),logLik))

rsquares<-unlist(lapply(list(MC1.1,MC1.2,MC1.3,MC1.4,MC1.5,MC1.6,MC1.7,MC1.8,MC1.9,MC1.10,MC2.1,MC2.2,MC2.3,MC2.4,MC2.5,MC2.6,MC2.7,MC2.8,MC2.9,MC2.10),function(x)cor(x$fitted.values+x$residuals,x$fitted.values)^2))

print(xtable(data.frame(Model=c(paste("MC1.",1:10,sep=""),paste("MC2.",1:10,sep="")),logLik=logLik,npar=npar,AIC=-2*logLik+2*npar,rsquares=rsquares)),type=table.type,html.table.attributes="border=1, bgcolor=#989898",comment=FALSE)

#print(xtable(rbind(AIC(MC1.1,MC1.2,MC1.3,MC1.4,MC1.5,MC1.6,MC1.7,MC1.8,MC1.9,MC1.10),AIC(MC2.1,MC2.2,MC2.3,MC2.4,MC2.5,MC2.6,MC2.7,MC2.8,MC2.9,MC2.10))),type=table.type,html.table.attributes="border=1, bgcolor=#989898",comment=FALSE)

# Calculate R2
#cor(MC1.4$fitted.values+MC1.4$residuals,MC1.4$fitted.values)^2
...
plot(brazil)
rect(environment.AF.2d$AF.Long2-1,environment.AF.2d$AF.Lat2-1,environment.AF.2d$AF.Long2+1,environment.AF.2d$AF.Lat2+1,
     col=adjustcolor(color.select(environment.AF.2d$pcoa.jac.neutral.total.1,ref=environment.AF.2d$pcoa.jac.neutral.total.1*environment.AF.2d$Long2/environment.AF.2d$Long2),alpha=.8))
...
par(op)
```
#### Fig 3. Projection of the species composition predicted by dispersal models on the map and comparison with the observed species distribution. The colors represent the similarity in species composition measured by the pairwise jaccard similarity index between sites summirized in one axis of a principal coordinates analysis (pcoa). Those grid cells with similar colors have a similar composition of species. (A) Observed species composition; (B) Species composition predicted by the environmental variables in the logistic models; (C) Species composition predicted by the Mid Domain model; (C) Species composition predicted by the neutral model. Shadded grid cells represent areas included in the neutral model but where actual data is not available.


####


  The mantel test results showed a stronger environmental effect when grouping the points in grid cells than when using the original data points (Table 5). The geographical distance was highly correlated with the species composition in both cases. Geographical distance was correlated with the species composition when the environment was partialed out, but the correlation was stronger in the unconstrained model (Table 5). The similarity in species composition was also weakly associated with the environment when controlling for geographical distance in the grouped data.
...
#reg.model.jac.dist<-predict(glm((1-similarity.jac)~geodist.2d,family = binomial(link=log)))
#reg.model.jac<-predict(lm(log(.000001+(1-similarity.jac))~geodist.2d))

#reg.model.jac<-predict(lm(log(.000001+(1-similarity.jac))~envdist.2d))
#reg.model.jac<-predict(glm((1-similarity.jac)~envdist.2d,family = binomial(link=log)))
#reg.model.jac<-predict(lm((similarity.jac)~envdist.2d))


#points(geodist.2d,exp(reg.model.jac)-.000001,pch=21,bg=adjustcolor(3,alpha=.2),col=NULL) #Lat

#points(geodist.2d,exp(reg.model.jac.dist),pch=21,bg=adjustcolor(3,alpha=.2),col=NULL) #Lat

points(geodist.2d,MS1.1$fitted.values,pch=21,bg=adjustcolor(3,alpha=.3),col=NULL) #Lat

#points(geodist.2d,1-(reg.model.jac),pch=21,bg=adjustcolor(3,alpha=.2),col=NULL) #Lat
#points(geodist.2d,1-as.dist(apply(similarity.logis.jac.ls[[2]],2,rowMeans)),pch=21,bg=adjustcolor(3,alpha=.2),col=NULL) #Lat
...
#lines(levels(factor(geodist.2d)),tapply(-.000001+exp(reg.model.jac),geodist.2d,mean),lwd=2,col=1)
#lines(levels(factor(geodist.2d)),tapply(1-(reg.model.jac),geodist.2d,mean),lwd=2,col=1)
lines(levels(factor(geodist.2d)),tapply(MS1.1$fitted.values,geodist.2d,mean),lwd=2,col=1)

#lines(levels(factor(geodist.2d)),tapply(1-as.dist(apply(similarity.logis.jac.ls[[1]],2,rowMeans)),geodist.2d,mean),lwd=2,col=1)
...
legend("topright",legend=c(paste("Logarithm",round(1-(MS1.0$deviance/MS1.0$null.deviance),3)),  paste("Environment",round(1-(MS1.1$deviance/MS1.1$null.deviance),3)),  paste("Neutral",round(1-(MS1.4$deviance/MS1.4$null.deviance),3)),paste("Mid Domain",round(1-(MS1.5$deviance/MS1.5$null.deviance),3))),title="R-squares:",col=c(1,1,4,2),lwd=2,lty=c(2,1,1,1),pch=c(NA,21,21,21),pt.bg=adjustcolor(c(1,3,4,2),alpha=.7),bty="n",pt.lwd=0,pt.cex=1.3)


1-(MS1.5$deviance/MS1.5$null.deviance)
...
```{r Fig_S3_jac_logis_in_the_map, echo=FALSE, message=FALSE,warning=FALSE}
...
```
#### Fig S3. Projection of the species composition predicted by dispersal models on the map and comparison with the observed species distribution. The colors represent the similarity in species composition measured by the pairwise jaccard similarity index between sites summirized in one axis of a principal coordinates analysis (pcoa). Those grid cells with similar colors have a similar composition of species. (A) Observed species composition; (B-Z) Species compostition predicted by logistic regressions of individual species against the environmental gradients.
...
```{r Fig_S5_Residuals_similarity,fig.width=9,fig.height=9,echo=FALSE,message=FALSE,results='hide',warning=FALSE}

par(mfrow=c(2,2))

plot(MS1.0$fitted.values,MS1.0$residuals,xlab="Fitted values",ylab="Residuals",cex.lab=1.5,pch=21,bg=1)

plot(geodist.2d,MS1.4$residuals,xlab="Fitted values",ylab="Residuals",cex.lab=1.5,pch=21,bg=1)

lines(smooth.spline(MS1.0$fitted.values,MS1.0$residuals))
abline(h=0,lwd=2,col=2)
title("A)",cex.main=1.5,adj=0)

plot(MS1.1$fitted.values,MS1.1$residuals,xlab="Fitted values",ylab="Residuals",cex.lab=1.5,pch=21,bg=1)
lines(smooth.spline(MS1.1$fitted.values,MS1.1$residuals))
abline(h=0,lwd=2,col=2)
title("B)",cex.main=1.5,adj=0)

plot(MS1.4$fitted.values,MS1.4$residuals,xlab="Fitted values",ylab="Residuals",cex.lab=1.5,pch=21,bg=1)
lines(smooth.spline(MS1.4$fitted.values,MS1.4$residuals))
abline(h=0,lwd=2,col=2)
title("C)",cex.main=1.5,adj=0)

plot(MS1.5$fitted.values,MS1.5$residuals,xlab="Fitted values",ylab="Residuals",cex.lab=1.5,pch=21,bg=1)
lines(smooth.spline(MS1.5$fitted.values,MS1.5$residuals))
abline(h=0,lwd=2,col=2)
...
plot(MC1.1$fitted.values,MC1.1$fitted.values+MC1.1$residuals)
plot(MC1.8$fitted.values,MC1.8$fitted.values+MC1.8$residuals)

abline(lm(MC1.1$fitted.values+MC1.1$residuals~MC1.1$fitted.values))
abline(0,1)

plot(MC1.4$fitted.values,MC1.4$fitted.values+MC1.4$residuals)
abline(lm(MC1.4$fitted.values+MC1.4$residuals~MC1.4$fitted.values))
abline(0,1,col=4)
