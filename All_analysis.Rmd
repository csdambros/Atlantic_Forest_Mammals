...
### Load required packages, import predefined functions and set default graphical parameters
...
### Importing data from the WorldClim (automatically downloads if necessary)

```{r Importing environmental layers from worldclim, results="hide", cache=TRUE}
...
#distance from the coast in each sampling unit
...
```{r Summarizing environmental data for each quadrant, results="hide"}
...
temp<-extract(wclim.clipped,extent(environment.2d[i,1]-1,environment.2d[i,1]+1,environment.2d[i,2]-1,environment.2d[i,2]+1))
environment.2d[i,paste("bio",1:19,sep="")]<-colMeans(temp,na.rm=TRUE)}
...
legend("left", legend=round(exp(sizes)) , pch = 21, col=0,pt.bg=adjustcolor(2,alpha=.5),pt.cex=sizes/3,y.intersp=exp(seq(log(1.2),log(1.5),length=divs)),x.intersp=3,bty="n")
...
neutral.AF<-anetwork(N,M,x[2],nruns=1000,stepshow=1000)
F1=neutral.AF$finalF
...
```{r neutral_theory_real_simulation, cache=TRUE}
...
similarity.simu<-array(NA,dim=c(nrow(environment.2d),nrow(environment.2d),reps2))
similarity.simu.jac<-array(NA,dim=c(nrow(environment.2d),nrow(environment.2d),reps2))
hill.simu<-matrix(NA,nrow(environment.AF.2d),reps2)
rich.simu<-matrix(NA,nrow(environment.AF.2d),reps2)
...
prcomp(apply(similarity.simu[,,round(.6*reps2):reps2],2,function(x)rowMeans(x)))$x[,1:3]

environment.AF.2d[environment.2d$matchin.AF.2d,paste("pcoa.jac.sim.",1:3,sep="")]<-#PCA.AF[,1:3]
prcomp(apply(similarity.simu.jac[,,round(.6*reps2):reps2],2,function(x)rowMeans(x)))$x[,1:3]
...
```{r, using sdm - logistic regression of each species, warning=FALSE}
